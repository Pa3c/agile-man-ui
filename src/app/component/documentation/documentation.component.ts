...
@ViewChild(CustomTextEditorComponent) customEditor:CustomTextEditorComponent;
  ...
  constructor(private router: Router,private documentationService:DocumentationService,private documentationVersionService: DocumentationVersionService,
    private route: ActivatedRoute,private fileService: FileService) { }

  ngOnInit(): void {
    this.route.params.subscribe(params => this.versionId = params['id']);
    this.documentationService.getNewestVersion(this.versionId).subscribe((success: DocumentationVersion) => {
  ...
  saveEditing(){
    ...
    this.documentationVersionService.create(docVersion).subscribe((success:DocumentationVersion)=>{
      this.router.navigateByUrl(`documentations/${this.resourceId}`);
    })
  }
